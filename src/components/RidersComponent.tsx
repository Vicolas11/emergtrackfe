...
const RidersComponent = ({ isAdmin = false }: ReqCompProps) => {
  const ridArrData = useAppSelector((state) => state.user.riderArrData);
  const isLoading = useAppSelector((state) => state.user.isLoading);
  const totalPgs = useAppSelector((state) => state.user.totalPagesRid);
  const successRid = useAppSelector((state) => state.user.successRid);
  const errRid = useAppSelector((state) => state.user.errorRid);
  ...
  useEffect(() => {
    console.log(errRid);
    dispatch(getAllRiders({ limit: 9, page: currentPg }));
  }, [dispatch, successRid, currentPg, errRid]);
      ...
      <div className={isAdmin ? styles.dashAdminTable : styles.dashTable}>
        {isLoading ? (
          <h1 className="text-[#09483D] font-bold text-2xl text-center">
            Rider data loading, Please Wait...
          </h1>
        ) : errRid ? (
          <h1 className="text-[#09483D] font-bold text-2xl text-center">
            An error occured...
