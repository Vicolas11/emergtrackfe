...
const DriversComponent = ({ isAdmin = false }: ReqCompProps) => {
  const driArrData = useAppSelector((state) => state.user.driverArrData);
  const isLoading = useAppSelector((state) => state.user.isLoading);
  const totalPgs = useAppSelector((state) => state.user.totalPagesDri);
  const successDri = useAppSelector((state) => state.user.successDri);
  const errDri = useAppSelector((state) => state.user.errorDri);
  ...
  useEffect(() => {
    dispatch(getAllDrivers({ limit: 9, page: currentPg }));
  }, [dispatch, successDri, currentPg]);
